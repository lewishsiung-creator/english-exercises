...
const BLOCKS = {
  ...
  /* Anita's own sentences, corrected. The mistake is shown as it was said and
...
function buildBanner() {
  ...
  const msg = typeof s === 'string' ? s
    : 'Sample session — the words here are a placeholder, not Anita’s own work. 範例課程：此處內容為範本，並非 Anita 的實際上課紀錄。';
...
const store = {
  get(k, fallback) {
    try { const v = localStorage.getItem(`anita.${k}`); return v === null ? fallback : v; }
    catch { return fallback; }
  },
  set(k, v) { try { localStorage.setItem(`anita.${k}`, v); } catch { /* private mode */ } },
};
