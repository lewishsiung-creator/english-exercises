/* Renders NOTEBOOK (content.js) and wires up the activities.
   ...
   - Sessions fold. The newest is open and the earlier ones are closed, because
     ...
     anchor (/aaron/#s3). The teacher panel opens the lot.
...
const BLOCKS = {
  ...
  /* Aaron's own sentences, corrected. The mistake is shown as it was said and
...
function buildBanner() {
  ...
  const msg = typeof s === 'string' ? s
    : 'Sample session — the words here are a placeholder, not Aaron’s own work. 範例課程：此處內容為範本，並非 Aaron 的實際上課紀錄。';
...
const store = {
  get(k, fallback) {
    try { const v = localStorage.getItem(`aaron.${k}`); return v === null ? fallback : v; }
    catch { return fallback; }
  },
  set(k, v) { try { localStorage.setItem(`aaron.${k}`, v); } catch { /* private mode */ } },
};
