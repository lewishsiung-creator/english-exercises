...
const BLOCKS = {
  ...
  question: (b) => `
    <div class="ask-card">
      ...
      <p class="ask-shape"><span class="ask-shape-lab">Shape 結構</span>
  ...
  /* Two clocks: thirty seconds to plan, then a count-up while he speaks. The
     speaking clock is the only number on the page that means anything, and it
     is not a score — it marks the band a Part 3 answer should land in. */
  clock: (b) => `
    <div class="activity clocks" data-act="clock">
      ${label(b.en, b.zh)}
      <div class="clock-row">
        <div class="clock-unit" data-mode="down" data-start="30">
          <p class="clock-name">Plan<em>計畫</em></p>
          <p class="clock-face">0:30</p>
          <p class="clock-note">thirty seconds, no writing after that<em>三十秒，之後不再動筆</em></p>
        ...
        <div class="clock-unit" data-mode="up" data-start="0">
          ...
          <p class="clock-note">aim to land between 0:40 and 1:15<em>目標落在 0:40 到 1:15 之間</em></p>
          <div class="clock-btns">
            <button class="chip clock-go">
              <span class="go">Start<em>開始</em></span>
              <span class="stop">Stop<em>停</em></span>
            </button>
            <button class="chip clock-zero">Reset<em>重設</em></button>
          </div>
        </div>
...
const store = {
  get(k, fallback) {
    try { const v = localStorage.getItem(`p3.${k}`); return v === null ? fallback : v; }
    catch { return fallback; }
  },
  set(k, v) { try { localStorage.setItem(`p3.${k}`, v); } catch { /* private mode */ } },
};
...
function tickClock(unit) {
  ...
  if (!down) {
    unit.classList.toggle('in', sec >= 40);
    unit.classList.toggle('over', sec >= 75);
  }
