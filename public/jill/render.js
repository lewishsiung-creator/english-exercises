/* Renders NOTEBOOK (content.js) and wires up the activities.
   ...
   - Entries fold. The newest session is open and everything else is closed,
     ...
     anchor (/jill/#s3). The teacher panel opens the lot.
...
/* The Traditional Chinese for one word-box word, from the shared map in
   `NOTEBOOK.glossary`. Falling back to an empty string rather than throwing is
...
const BLOCKS = {
  ...
  /* A divider INSIDE a session, for material that arrives already cut into
     parts. `group` bands separate one kind of entry from another across the
     whole document; this separates Part 1 from Part 2 within one lesson, which
     is what a five-part worksheet needs and what a one-topic session does not.
     Signage, like the contents list and the focus chips, so both languages
     show at once and there is no 中 chip to tap. */
  part: (b) => `
    <div class="part-head">
  ...
  /* Phrase cards. English is face up; the Chinese and an example sentence are
     ...
     `meaning` is optional and is Jill's class notes speaking: they give an
     English definition under every word, and throwing it away to keep the card
     shape would lose the better half of the teaching. It sits ABOVE the
     Chinese on the back, so the English explanation is what she meets first
     and the gloss is the safety net under it. Cards written without one — a
     collocation, a whole sentence — render exactly as before. */
  phrases: (b) => `
    <div class="phrases">
      ${label(b)}
      <ul class="cards">
        ${b.items.map((i) => `
          <li class="card" tabindex="0" role="button" aria-expanded="false">
            ...
            <div class="card-back">
              ${i.meaning ? `<p class="card-mean">${text(i.meaning)}</p>` : ''}
  ...
  /* Jill's own sentences, corrected. The mistake is shown as it was said and
...
/* The order the page DISPLAYS, which is not the order the data is written in.
   ...
   content.js is authored oldest-first, because that is how you add to it: a
   new session goes at the bottom of the array and inherits the next number. But a notebook a term deep is read newest-first —
   ...
   (/jill/#s5) still points at the same session.

   Grouped entries — anything carrying a `group` field — are NOT reversed. This
   page has none yet; the machinery is kept so undated practice papers could be
   appended later without touching the renderer. */
...
function buildBanner() {
  ...
  const msg = typeof s === 'string' ? s
    : 'Sample session — the words here are a placeholder, not their own work. 範例課程：此處內容為範本，並非實際上課紀錄。';
    ...
    <section class="session ${open ? '' : 'folded'}" id="${s.id}">
      <h2 class="session-head">
        <button class="session-btn" aria-expanded="${open}" aria-controls="body-${s.id}">
          ...
          <span class="titles">
            <span class="date">${text(s.dateEn)}<em>${text(s.dateZh || '')}</em></span>
...
const store = {
  get(k, fallback) {
    try { const v = localStorage.getItem(`jill.${k}`); return v === null ? fallback : v; }
    catch { return fallback; }
  },
  set(k, v) { try { localStorage.setItem(`jill.${k}`, v); } catch { /* private mode */ } },
};
