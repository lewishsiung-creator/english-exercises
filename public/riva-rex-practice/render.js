/* Renders NOTEBOOK (content.js) and wires up the activities.
   ...
   - Exercises fold. The first is open and the rest are closed, because
     ...
     anchor (/riva-rex-practice/#e3). The teacher panel opens the lot.
   - Which sessions are open is NOT remembered between loads. A reload is a
     clean start, the same as every other page here: the first exercise, open.
...
const BLOCKS = {
  ...
  /* Riva or Rex's own sentences, corrected. The mistake is shown as it was said and
...
function buildNav() {
  return `
    <nav class="toc" aria-label="Exercises">
      <p class="toc-head">Exercises 練習</p>
      <ul>
...
/* Open the exercise named in the URL, or the FIRST one. This is the one place
   the practice page parts company with the notebook: a notebook opens on the
   newest session, because the newest lesson is the one you came back for, but
   a set of exercises has no newest — it has a first, and opening anywhere else
   makes the page look as though it starts in the middle. A hash that matches
   nothing falls back to the same place, so an old link cannot land on a page
   with everything shut. */
...
const store = {
  get(k, fallback) {
    try { const v = localStorage.getItem(`rivarexpractice.${k}`); return v === null ? fallback : v; }
    catch { return fallback; }
  },
  set(k, v) { try { localStorage.setItem(`rivarexpractice.${k}`, v); } catch { /* private mode */ } },
};
