/* Renders NOTEBOOK (content.js) and wires up the activities.
   ...
   - Sessions fold. The newest is open and the earlier ones are closed, because
     ...
     anchor (/mia/#s1). The teacher panel opens the lot.
   ...
   - Sessions here are DATED. `dateEn`/`dateZh` remain optional in this renderer,
     inherited from /anna/, whose sessions carry no date: present, they draw the
     date line and the contents-list subtitle for that entry; absent, both are
     omitted rather than left as empty elements taking up space. Give every
     session here both, so the array's order and the dates agree.
...
const BLOCKS = {
  /* A divider INSIDE one session. Ken's first session runs A–E — his own
     sentences, the polished model, the collocations, the patterns, the
     discussion — and a session with that many turns in it needs a heading one
     level below the session's own, so Part B does not read as a new lesson. A
     short session built from a conversation alone should not use it. Signage:
     both languages show at once and there is no 中 chip to tap. */
  part: (b) => `
    <div class="part-head">
  ...
  /* A teaching paragraph, spoken aloud on request.
     ...
     weeks later, when the student is looking for one idea rather than reading the lot.
  ...
  /* Phrase cards. English is face up; the Chinese and an example sentence are
     behind a tap, so the meaning can be tried first. There is no `hint` here,
     deliberately: a set of cards needs no instructions. Anything that has to be
     said before a card set goes in a `lead` or a `note` above it, not in a
     `hintEn` that this template would silently drop. */
  phrases: (b) => `
    <div class="phrases">
      ${label(b)}
      <ul class="cards">
  ...
  /* A quiz question about the reading. Unlike `gap` there is no blank to fill,
     ...
     itself is the explanation and `why` should be left off. Where it asks the student
  ...
  /* Mia's own sentences, corrected. The mistake is shown as it was said and
  ...
  /* Have-it / need-it cards, for auditing something against a list.
     ...
     habits and that Chinese is simply wrong. Set all four whenever the items
     are not people; the defaults stay for the case the block was written for. */
  cards: (b) => `
...
function buildBanner() {
  ...
  const msg = typeof s === 'string' ? s
    : 'Sample session — the words here are a placeholder, not Mia’s own work. 範例課程：此處內容為範本，並非 Mia 的實際上課紀錄。';
...
const store = {
  get(k, fallback) {
    try { const v = localStorage.getItem(`mia.${k}`); return v === null ? fallback : v; }
    catch { return fallback; }
  },
  set(k, v) { try { localStorage.setItem(`mia.${k}`, v); } catch { /* private mode */ } },
};
