/* Renders NOTEBOOK (content.js) and wires up the activities.
   ...
   - Sessions fold. The newest is open and the earlier ones are closed, because
     ...
     anchor (/ken/#s3). The teacher panel opens the lot.
...
const BLOCKS = {
  ...
  /* A quiz question about the reading. Unlike `gap` there is no blank to fill,
     ...
     itself is the explanation and `why` should be left off. Where it asks him
  ...
  /* Ken's own sentences, corrected. The mistake is shown as it was said and
...
function buildBanner() {
  ...
  const msg = typeof s === 'string' ? s
    : 'Sample session — the words here are a placeholder, not Ken’s own work. 範例課程：此處內容為範本，並非 Ken 的實際上課紀錄。';
...
const store = {
  get(k, fallback) {
    try { const v = localStorage.getItem(`ken.${k}`); return v === null ? fallback : v; }
    catch { return fallback; }
  },
  set(k, v) { try { localStorage.setItem(`ken.${k}`, v); } catch { /* private mode */ } },
};
